...
function renderNav(status) {
  ...
  if (status.logged_in) {
    nav.innerHTML = `
      <span class="nav-username text-muted" style="font-size:0.82rem">${escHtml(status.player_name || '')}</span>
...
function renderProfile(me) {
  ...
  stats.innerHTML = `
    <div class="pstat"><div class="pstat-label">Global Rank</div><div class="pstat-val cyan">${rank}</div></div>
    <div class="pstat"><div class="pstat-label">Country Rank</div><div class="pstat-val">${countryRank}</div></div>
    <div class="pstat"><div class="pstat-label">PP</div><div class="pstat-val violet">${pp}pp</div></div>
    <div class="pstat"><div class="pstat-label">Avg Acc</div><div class="pstat-val">${acc}</div></div>
    <div class="pstat"><div class="pstat-label">Plays</div><div class="pstat-val">${totalPlays}</div></div>
  `;
}
...
function friendCardHtml(f, snipeCount) {
  ...
  return `
    <div class="glass-card friend-card" data-friend-id="${f.id}" title="View snipe targets for ${escHtml(f.name)}">
      <div class="friend-top">
        <img class="friend-avatar" src="${escHtml(f.avatar || '')}" alt="" onerror="this.src='data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 40 40%22><circle cx=%2220%22 cy=%2220%22 r=%2220%22 fill=%22%23333%22/></svg>'">
        <div>
          <div class="friend-name">${escHtml(f.name || 'Player')}</div>
          <div class="friend-rank">${rank}</div>
        </div>
        <div class="friend-pp">${pp}<span style="font-size:0.7rem;opacity:0.6">pp</span></div>
      </div>
      <div class="friend-bottom">
        <div id="snipe-count-${f.id}">${snipeHtml}</div>
        <button class="btn-snipe" data-friend-id="${f.id}">🎯 Snipe</button>
      </div>
    </div>
  `;
}
...
function scoreCardHtml(s) {
  ...
  return `
    <div class="glass-card score-card">
      <img class="score-cover" src="${escHtml(cover)}" alt="" onerror="this.style.background='linear-gradient(135deg,#1a1a2e,#16213e)'">
      <div class="score-info">
        <div class="score-name" title="${escHtml(name)}">${escHtml(name)}</div>
        <div class="score-diff">${escHtml(diff)}</div>
        <div class="score-stats">
          <div class="score-stat"><span class="label">ACC</span><span class="val text-cyan">${acc}</span></div>
          ${pp ? `<div class="score-stat"><span class="label">PP</span><span class="val text-violet">${pp}</span></div>` : ''}
        </div>
      </div>
...
function snipeItemHtml(t) {
  ...
  return `
    <a class="snipe-item ${difficulty}" href="${blUrl}" target="_blank" rel="noopener" title="Open on BeatLeader">
      <img class="snipe-cover" src="${escHtml(t.coverImage || '')}" alt="" onerror="this.style.background='linear-gradient(135deg,#1a1a2e,#16213e)'">
      <div class="snipe-info">
        <div class="snipe-song">${escHtml(t.songName || 'Unknown')}</div>
        <span class="snipe-diff-badge">${escHtml(t.difficulty || '')}</span>
        <div class="snipe-acc-compare">
          <span class="acc-them">Them: ${t.friendAcc?.toFixed(2) ?? '—'}%</span>
          ${myAccHtml}
        </div>
      </div>
      <div class="snipe-gap-pill ${gapClass}">${gapLabel}</div>
    </a>
  `;
}
